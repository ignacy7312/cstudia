...
int main() {
	...
	//3. copy_memory test
	int source = 2000;
	assert(copy_memory(&memory_sector, get_left_fence_address(&memory_sector) + 1, &source, sizeof(source)) == NO_ERROR);
	assert(read_memory(&memory_sector, get_left_fence_address(&memory_sector) + 1, &buffer) == NO_ERROR);
	assert(buffer == source);
	
	assert(copy_memory(&memory_sector, get_right_fence_address(&memory_sector) - 1, &source, sizeof(source)) == NO_ERROR);
	assert(read_memory(&memory_sector, get_right_fence_address(&memory_sector) - 1, &buffer) == NO_ERROR);
	assert(buffer == source);

	assert(copy_memory(&memory_sector, get_left_fence_address(&memory_sector) - 1, &source, sizeof(source)) == ADDRESS_OUT_OF_RANGE);
